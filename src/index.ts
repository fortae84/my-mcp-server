...
server.registerTool(
    'time',
    {
        ...
        inputSchema: z.object({
            city: z
                ...
                .default('all')
                .describe(
                    '조회할 도시 (seoul, new_york, chicago, denver, los_angeles, all)'
                )
        }),
        outputSchema: z.object({
            content: z
    ...
    async ({ city }) => {
        ...
        if (city === 'all') {
            const lines = Object.values(CITY_TIMEZONES).map(
                ({ timezone, label }) => `${label}: ${formatTime(timezone)}`
            )
            text = lines.join('\n')
        } else {
...
server.registerTool(
    'geocode',
    {
        ...
        inputSchema: z.object({
            query: z.string().describe('검색할 도시명 또는 주소 (예: Seoul, 서울, 1600 Pennsylvania Ave NW Washington DC)')
        }),
        outputSchema: z.object({
            content: z
                .array(
                    z.object({
                        type: z.literal('text'),
                        text: z.string().describe('위도/경도 결과')
                    })
                )
                .describe('위도/경도 결과')
        })
    ...
    async ({ query }) => {
        ...
        if (!results || results.length === 0) {
            text = `검색 결과가 없습니다: ${query}`
        } else {
            const { lat, lon, display_name } = results[0]
            text = `장소: ${display_name}\n위도: ${lat}\n경도: ${lon}`
        }
...
function describeWeatherCode(code: number): string {
    return WMO_WEATHER_CODES[code] ?? `알 수 없음 (코드: ${code})`
}
...
server.registerTool(
    'get-weather',
    {
        ...
        inputSchema: z.object({
            latitude: z
                ...
                .min(-90)
                .max(90)
                .describe('위치의 위도 (-90 ~ 90)'),
            longitude: z
                .number()
                .min(-180)
                .max(180)
                .describe('위치의 경도 (-180 ~ 180)'),
            forecast_days: z
                .number()
                .int()
                .min(1)
    ...
    async ({ latitude, longitude, forecast_days }) => {
        ...
        const response = await fetch(url.toString(), {
            headers: { 'User-Agent': 'MCP-Weather-Tool/1.0' }
        })

        if (!response.ok) {
            const errorText = `날씨 API 오류: HTTP ${response.status}`
            return {
                content: [{ type: 'text' as const, text: errorText }],
                structuredContent: {
                    content: [{ type: 'text' as const, text: errorText }]
...
server.registerTool(
    ...
    async ({ prompt, num_inference_steps }) => {
        ...
        if (!token) {
            return {
                content: [
                    {
                        type: 'text' as const,
                        text: '오류: HF_TOKEN 환경변수가 설정되지 않았습니다.'
                    }
                ]
            }
        }
        ...
        try {
            ...
            // dataUrl 형식: "data:<mimeType>;base64,<data>"
    ...
    'code-review',
    {
        ...
        argsSchema: {
            ...
            language: z
                .string()
                .optional()
                .describe('프로그래밍 언어 (예: TypeScript, Python, Java 등)'),
            focus: z
...
다음 항목을 포함하여 리뷰해주세요:
1. **버그 및 잠재적 오류** - 런타임 에러, 엣지 케이스, 논리적 오류
2. **성능** - 불필요한 연산, 메모리 낭비, 최적화 기회
3. **보안** - 취약점, 입력 검증, 인증/인가 문제
4. **가독성 & 유지보수성** - 네이밍, 구조, 주석, 중복 코드
5. **개선 제안** - 구체적인 수정 코드 예시 포함
...
server.registerResource(
    ...
    async (uri) => {
        ...
        const uptimeStr = `${uptimeHour}시간 ${uptimeMin % 60}분 ${uptimeSec % 60}초`
        ...
        const info = {
            ...
            tools: [
                ...
                { name: 'calc', description: '두 숫자와 연산자를 입력받아 사칙연산 결과를 반환합니다.' },
                { name: 'time', description: '특정 도시의 현재 시간을 반환합니다.' },
                { name: 'geocode', description: '도시 이름이나 주소를 입력받아 위도와 경도 좌표를 반환합니다.' },
                { name: 'get-weather', description: '위도/경도 좌표를 기반으로 현재 날씨와 일별 예보를 반환합니다.' }
            ],
            system: {
